...
test('운영 취지 공지문에 229개 센터장 문구가 있다', () => {
  ...
  assert.equal(CENTER_PURPOSE_SECTIONS.length, 4);
  ...
  assert.match(CENTER_PURPOSE_SECTIONS[3].heading, /향후 추진 계획/);
  assert.match(CENTER_PURPOSE_SECTIONS[3].stages[0].title, /1단계/);
  assert.match(CENTER_PURPOSE_SECTIONS[3].stages[2].items.join(' '), /RTO/);
});
