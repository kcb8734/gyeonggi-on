...
test('build:aab script prints gradle and dist AAB paths', () => {
  ...
  assert.match(source, /PLAY_UPLOAD_CERT_SHA256/);
  assert.match(source, /PLAY_UPLOAD_CERT_MD5/);
  assert.match(source, /30:70:7C:14:A2:AA:1B:AD:06:5C:E7:CC:79:AC:02:BB:9B:D8:2C:42/);
  assert.match(source, /46:75:CA:CC:9B:12:17:1F:E2:06:87:2F:F3:86:FD:77/);
  assert.doesNotMatch(source, /기존 키스토어가 없어 업로드 키를 새로 만듭니다/);
  assert.doesNotMatch(source, /-genkeypair/);
  assert.doesNotMatch(source, /업로드 키 재설정이 승인된 뒤에만/);
});
