...
export type GgCultureTag = (typeof TAGS)[number];

export type GgCultureRow = Record<GgCultureTag, string>;

export interface GgCultureParseResult {
  ok: boolean;
  code: string;
  message: string;
  total: number;
...
export interface PersistableGgFestival {
  contentId: string;
  title: string;
  category: string;
...
export function ymdDash(value: string) {
...
export function encodeMediaUrl(value: string) {
...
export function feeLabel(value: string) {
  const text = String(value || '').trim();
...
export function contentIdFromRow(row: Partial<GgCultureRow>) {
...
export function parseGgCultureXml(xml: string): GgCultureParseResult {
  ...
  const rows: GgCultureRow[] = [];
  const rowRe = /<row\b[^>]*>([\s\S]*?)<\/row>/gi;
  let match = rowRe.exec(text);
  while (match) {
    const block = match[1];
    const row = {} as GgCultureRow;
...
export function countCategories(rows: Array<{ CATEGORY_NM?: string; category?: string }>) {
...
export function toPersistableFestival(row: GgCultureRow): PersistableGgFestival | null {
