...
export type IfacCultureTag = (typeof TAGS)[number];
export type IfacCultureRow = Record<IfacCultureTag, string>;

export interface IfacCultureParseResult {
  ok: boolean;
  code: string;
  message: string;
  total: number;
...
export interface PersistableIfacFestival {
  contentId: string;
  title: string;
  category: string;
...
export function ifacMetro(sido: string, gugun: string) {
...
export function contentIdFromRow(row: Partial<IfacCultureRow>) {
...
export function parseIfacCultureXml(xml: string): IfacCultureParseResult {
...
export function countCategories(rows: Array<{ category?: string }>) {
...
export function toPersistableFestival(row: IfacCultureRow): PersistableIfacFestival | null {
