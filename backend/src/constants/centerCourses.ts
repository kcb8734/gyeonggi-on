...
export function upsertCenterCourse(input: CenterCourseInput): { ok: true; data: CenterLocalCourse } | { ok: false; message: string } {
...
export function centerCourseToFestivalCourse(course: CenterLocalCourse): FestivalCourse {
  ...
  return {
    ...
    itinerary: stops.map((item) => ({
      ...
      latitude: item.stop.latitude ?? 0,
      longitude: item.stop.longitude ?? 0,
    })),
