...
const SLUG_OVERRIDES = {
  춘천: 'chuncheon', 수원: 'suwon', 성남: 'seongnam', 파주: 'paju', 용인: 'yongin', 고양: 'goyang',
  종로: 'jongno', 강남: 'gangnam', 마포: 'mapo', 해운대: 'haeundae', 연수: 'yeonsu', 강릉: 'gangneung',
  여수: 'yeosu', 제주: 'jeju', 보령: 'boryeong', 통영: 'tongyeong', 수성: 'suseong', 동구: 'donggu', 중구: 'junggu',
};

function shortLocalityName(label) {
  return String(label || '')
...
export function localityWebSlug(label) {
  const short = shortLocalityName(label);
  return SLUG_OVERRIDES[short] || romanizeHangul(short).toLowerCase().replace(/[^a-z0-9]+/g, '') || 'center';
}

export function dedicatedCenterName(region, label) {
  return `${GOV_NAME[normalizeMetroId(region)] || ''} ${shortLocalityName(label)} 전담센터`.trim();
}

export function directorTitleFor(regionLabel, label) {
  return `${regionLabel} ${shortLocalityName(label)}센터장`;
}

export function websiteForLocality(label) {
  return `kdanji.com/${localityWebSlug(label)}`;
}

const SELECTED_SEED = {
  'GYEONGGI:수원시': { name: '박서연', title: '경기온 수원 센터장', phone: '031-120', email: 'suwon@kdanji.com', intro: '수원화성 축제와 영동시장 상가를 잇는 현장 센터를 운영합니다.', address: '경기도 수원시 팔달구 정조로 825', website: 'kdanji.com/suwon' },
  'GYEONGGI:성남시': { name: '이준호', title: '경기온 성남 센터장', phone: '031-120', email: 'seongnam@kdanji.com', intro: '성남 모란시장과 지역 축제를 상생 쿠폰으로 연결합니다.', address: '경기도 성남시 수정구 산성대로 277', website: 'kdanji.com/seongnam' },
  'GYEONGGI:파주시': { name: '최민정', title: '경기온 파주 센터장', phone: '031-120', email: 'paju@kdanji.com', intro: '임진각·장단콩 축제와 문산·금촌 시장을 함께 안내합니다.', address: '경기도 파주시 시청로 50', website: 'kdanji.com/paju' },
  'SEOUL:종로구': { name: '김하늘', title: '서울온 종로 센터장', phone: '02-120', email: 'jongno@kdanji.com', intro: '광장시장과 종로 거리예술 축제를 잇는 도심 센터입니다.', address: '서울특별시 종로구 종로 1', website: 'kdanji.com/jongno' },
  'SEOUL:강남구': { name: '정우성', title: '서울온 강남 센터장', phone: '02-120', email: 'gangnam@kdanji.com', intro: '강남 축제 현장과 인근 소상공인 할인을 매칭합니다.', address: '서울특별시 강남구 강남대로 396', website: 'kdanji.com/gangnam' },
  'BUSAN:부산-해운대구': { name: '한지민', title: '부산온 해운대 센터장', phone: '051-120', email: 'haeundae@kdanji.com', intro: '해운대 축제와 자갈치·구남로 상권을 연결합니다.', address: '부산광역시 해운대구 해운대로 620', website: 'kdanji.com/haeundae' },
  'INCHEON:인천-연수구': { name: '오세린', title: '인천온 연수 센터장', phone: '032-120', email: 'yeonsu@kdanji.com', intro: '송도 축제와 신포시장 먹거리를 안내합니다.', address: '인천광역시 연수구 원인재로 115', website: 'kdanji.com/yeonsu' },
  'GANGWON:강릉시': { name: '윤바다', title: '강원온 강릉 센터장', phone: '033-120', email: 'gangneung@kdanji.com', intro: '강릉커피축제와 중앙시장 상가를 함께 소개합니다.', address: '강원특별자치도 강릉시 강릉대로 33', website: 'kdanji.com/gangneung' },
  'JEONNAM:전남-여수시': { name: '문가영', title: '전남온 여수 센터장', phone: '061-120', email: 'yeosu@kdanji.com', intro: '여수밤바다 축제와 교동시장을 잇습니다.', address: '전라남도 여수시 시청로 1', website: 'kdanji.com/yeosu' },
  'JEJU:제주시': { name: '고은섬', title: '제주온 제주 센터장', phone: '064-120', email: 'jeju@kdanji.com', intro: '들불축제와 동문시장 상생 쿠폰을 현장 안내합니다.', address: '제주특별자치도 제주시 문연로 6', website: 'kdanji.com/jeju' },
};

export const SELECTED_DIRECTORS = { ...SELECTED_SEED };

const SEED_REVIEWING = new Set([
  'GYEONGGI:용인시', 'GYEONGGI:고양시', 'SEOUL:마포구', 'BUSAN:부산-중구',
  'DAEGU:대구-수성구', 'GWANGJU:광주-동구', 'CHUNGNAM:충남-보령시', 'GYEONGNAM:경남-통영시',
]);

const applications = [];
const localityReview = new Map();

export function localityKey(region, localityId) {
  return normalizeMetroId(region) + ':' + localityId;
}

function localityMeta(key) {
  const parts = String(key).split(':');
  const region = normalizeMetroId(parts[0]);
  const localityId = parts.slice(1).join(':');
  const loc = (METRO_LOCALITIES[region] || []).find((row) => row.id === localityId);
  return {
    region,
    localityId,
    label: (loc && loc.label) || localityId,
    regionLabel: (REGION_META[region] && REGION_META[region].label) || region,
  };
}
...
export function applyCenterDirector(input) {
  const name = String((input && input.name) || '').trim();
  const age = String((input && input.age) || '').trim();
  const phone = String((input && input.phone) || '').trim();
  const address = String((input && input.address) || '').trim();
  const career = String((input && input.career) || '').trim();
  const intro = String((input && input.intro) || '').trim();
  const key = String((input && input.localityKey) || '');
...
export function applyBusinessCard(id) {
  const row = applications.find((item) => item.id === id);
  if (!row) return { ok: false, message: '지원서를 찾을 수 없습니다.' };
  const meta = localityMeta(row.localityKey);
...
function applicantCountFor(key) {
...
export function listCenterLocalities(region) {
  ...
  return keys.flatMap((metro) => {
    ...
    return (METRO_LOCALITIES[metro] || []).map((loc) => {
      ...
      return {
        ...
        label: loc.label,
        region: metro,
        regionLabel: (meta && meta.label) || metro,
        status,
